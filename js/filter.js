...
const getAllCheckedCheckboxes = () => {
  ...
  const checkboxesChecked = filterFormElement.querySelectorAll(
...
const filterType = (item) =>
  typeOfHouseElement.value === DEFAULT_VALUE ||
  typeOfHouseElement.value === item.offer.type;

const filterPrice = (item) =>
  housePriceElement.value === DEFAULT_VALUE ||
  (item.offer.price < 10000 && housePriceElement.value === 'low') ||
  (item.offer.price >= 10000 &&
    item.offer.price <= 50000 &&
    housePriceElement.value === 'middle') ||
  (item.offer.price > 50000 && housePriceElement.value === 'high');

const filterRooms = (item) =>
  numberOfRoomsElement.value === DEFAULT_VALUE ||
  +numberOfRoomsElement.value === item.offer.rooms;

const filterGuests = (item) =>
  amountOfGuestElement.value === DEFAULT_VALUE ||
  +amountOfGuestElement.value === item.offer.guests;
